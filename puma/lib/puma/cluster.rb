  ...
  class Cluster < Runner
    ...
    def stop_workers
      ...
      begin
        loop do
          wait_workers
          bread if @workers.reject { |w| w.pid.nil? }.empty?
          sleep 0.2
        end
      rescue Interrupt
    ...
    def spawn_worker(idx, master)
      ...
      if !pid
        log "! Complete inability to spawn worker"
    ...
    def next_worker_index
      occupied_indexes = @workers.map(&:index)
      idx = 0
      idx += 1 until !occupied_indexes.include?(idx)
      idx
    end
    ...
    def check_workers
      ...
      if all_workers_booted?
        ...
        if w
          log "- Stopping #{x.pid} for phased upgrade..."
          unless w.term?
            w.term
